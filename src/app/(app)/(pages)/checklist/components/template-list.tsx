    ...
    () =>
      Array.from({ length: Math.min(5, limit) }).map((_, i) => (
        <TableRow key={`skeleton-${i}`} className="animate-pulse">
          <TableCell className="h-10"><div className="h-3 w-44 bg-muted rounded" /></TableCell>
          <TableCell><div className="h-3 w-20 bg-muted rounded" /></TableCell>
          <TableCell><div className="h-3 w-10 bg-muted rounded" /></TableCell>
          <TableCell><div className="h-3 w-24 bg-muted rounded" /></TableCell>
          <TableCell><div className="h-6 w-12 bg-muted rounded" /></TableCell>
          <TableCell className="text-right"><div className="h-6 w-6 bg-muted rounded-full ml-auto" /></TableCell>
        </TableRow>
      )),
    [limit]
  );
  ...
  return (
    <Card>
      ...
      <CardContent className="min-h-[190px] -mt-[24px] px-4 pb-4 pt-0 relative">
        ...
        <div
          className={`${loading ? "opacity-100" : ""} transition-all opacity-0 h-0.5 bg-slate-400 w-full overflow-hidden absolute left-0 right-0 top-0`}
        >
          <div
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-44">
                        <DropdownMenuItem onClick={() => { setSelected(c); setOpenView(true); }}>
      ...
      <Dialog open={openView} onOpenChange={setOpenView}>
        <DialogContent className="w-[95vw] max-w-3xl p-0" onOpenAutoFocus={(e) => e.preventDefault()}>
          <div className="grid w-full max-h-[90vh] grid-rows-[auto_1fr]">
            <DialogHeader className="px-4 sm:px-6 pt-4">
              <DialogTitle>{selected?.nome}</DialogTitle>
              {selected?.descricao && (
                <CardDescription className="pt-1">{selected.descricao}</CardDescription>
              )}
            </DialogHeader>

...
export async function PUT(
  ...
  try {
    ...
    // 🔎 Regra: só deixa definir responsável quando a OS está em ORCAMENTO_APROVADO
    // (ajusta se seu status for outro)
    ...
    // aqui você pode decidir se permite também em EM_ANDAMENTO
    if (status !== "ORCAMENTO_APROVADO") {
      return NextResponse.json(
        {
          error:
            "Só é possível definir responsáveis quando a OS está em ORCAMENTO_APROVADO (antes de iniciar).",
        },
        { status: 400 }
      );
    }
    ...
    if (!upd_res.data) {
      return NextResponse.json(
        { error: "Serviço não encontrado na OS" },
        { status: 404 }
      );
    }
    ...
    console.error(
      "PUT /api/ordens/[id]/servicos/[servicoId]/responsavel",
      e
    );
    return NextResponse.json(
      { error: e?.message ?? "Erro ao atualizar responsável" },
      { status: 500 }
    );
  }
}
